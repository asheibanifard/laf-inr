...
class LAFINRDataset(Dataset):
    ...
    def __init__(
        ...
        print('reconstruction flag is:', self.recflag)
    ...
    def __getitem__(self, idx: int):
        ...
        # Create LR version for reconstruction task
        C, Hs, Ws = hr.shape
        
        if self.scale == 1:
            # Reconstruction task: remove high frequencies via more aggressive filtering
            if self.recflag:
                # More aggressive frequency filtering for meaningful reconstruction
                # Method 1: Stronger bicubic downsampling + upsampling
                downsample_factor = 4  # Reduced from 8 - less aggressive frequency removal
                H_low, W_low = max(16, Hs // downsample_factor), max(16, Ws // downsample_factor)
                
                # Downsample to remove high frequencies  
                lr_temp = F.interpolate(hr.unsqueeze(0),
                                      size=(H_low, W_low),
                                      mode='bicubic',
                                      align_corners=False)
                
                # Upsample back to original size (band-limited version)
                lr = F.interpolate(lr_temp,
                                 size=(Hs, Ws),
                                 mode='bicubic',
                                 align_corners=False).squeeze(0)
                
                # Additional Gaussian blur to further reduce high frequencies
                sigma = 1.0  # Reduced from 2.0 - gentler blur
                kernel_size = 7  # Reduced from 9 - smaller blur kernel
                
                # Create Gaussian kernel
                kernel_1d = torch.exp(-0.5 * (torch.arange(kernel_size).float() - kernel_size // 2) ** 2 / sigma ** 2)
                ...
                lr_padded = F.pad(lr.unsqueeze(0), (padding, padding, padding, padding), mode='reflect')
                lr = F.conv2d(lr_padded, kernel, groups=C, padding=0).squeeze(0)
                
            else:
                # No frequency filtering - use original
                lr = hr.clone()
        else:
            # Traditional super-resolution task
