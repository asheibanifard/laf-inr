...
class QueryBuilder(nn.Module):
    ...
    def forward(self, pe: torch.Tensor, canvas_scalar: torch.Tensor) -> torch.Tensor:
        """
        pe:            [B,Hs,Ws,Pe] positional encodings
        canvas_scalar: [B,1,Hs,Ws]  scalar canvas value at each (x,y)
        returns:       [B, Nq, D] where Nq = Hs*Ws
        """
        B, Hs, Ws, Pe = pe.shape
        v = canvas_scalar.permute(0, 2, 3, 1)  # [B,Hs,Ws,1]
